...
| Public npm install works | clean consumer installs `@service-lasso/service-lasso` from `registry.npmjs.org` without GitHub Packages auth | Blocked | 2026-04-24: tracked as issue `#80`. Code/docs/workflows now target npmjs with `NPM_TOKEN`; unauthenticated `npm view @service-lasso/service-lasso version --registry=https://registry.npmjs.org` currently returns `E404` until the protected-branch publish runs after that secret exists. |
...
| Missing npm publish token | protected-branch package workflow fails before publish with actionable guidance | Pending | Issue `#80` adds an explicit workflow guard for missing `NPM_TOKEN`; proof requires a workflow run on `main` or a controlled workflow dry run. |
...
- 2026-04-24: issue `#80` moved the default consumer package path from GitHub Packages to public npmjs under `@service-lasso/service-lasso`; protected-branch publish proof is blocked until `NPM_TOKEN` is configured.
