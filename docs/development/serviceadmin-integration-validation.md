...
4. Validate the current dashboard / services / service-detail consumer model against the runtime.
   status: in_progress

5. Record any remaining consumer-model gaps as explicit follow-up work before calling the integration slice complete.
   status: in_progress
...
- `npm test`
- new route coverage in `tests/operator-data.test.js`
...
- Those pages do not yet consume the runtime as their primary data source, even when `VITE_SERVICE_LASSO_API_BASE_URL` is configured.

## Next recommended slice

The next clean delivery step is:

**add a bounded runtime-backed dashboard adapter in `lasso-@serviceadmin` for the current `service-lasso` service/detail/network/variables/logs surfaces, then rerun the consumer validation against the demo/runtime shape.**
