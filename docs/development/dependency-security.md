...
`adm-zip` 0.6.0 remains in the vulnerable `>=0.5.9` range for destination-symlink overwrite (GHSA-vwc7-r8mq-g2x9). Downgrading to 0.5.8 clears that advisory but reintroduces high-severity GHSA-xcpc-8h2w-3j85, so no published `adm-zip` release currently satisfies a zero-finding production audit. `@modelcontextprotocol/sdk` transitively resolves `hono` below 4.13.5; the lockfile forces `hono` 4.13.7 so MCP HTTP transport stays on the patched release without changing the SDK pin.
