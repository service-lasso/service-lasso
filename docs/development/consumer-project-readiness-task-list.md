...
Service Lasso is consumer-ready after the `develop` -> `main` promotion workflows pass and publish the timestamped release/package version.
...
- `CONSUMER-006`: promote `develop` to `main`, wait for release/package workflows, and record final release evidence on issue `#58`.
