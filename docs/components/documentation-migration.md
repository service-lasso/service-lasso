...
| Secrets Broker | Core already has setup, recovery, secret-access, and capability references | The exact-source audit is complete; [#1420](https://github.com/service-lasso/service-lasso/issues/1420) owns the 14 remaining operator-guide candidates |
| Other service and app-template repositories | On 15 September 2026, develop-branch API checks returned 23 source revisions and 19 unavailable/inaccessible results across the 42 inventoried repositories | [Core #1282](https://github.com/service-lasso/service-lasso/issues/1282) is closed after merged audit #1417. Its refreshed 169-path packet has 48 owned migration candidates and 121 retained references; #1418/#1419/#1420 carry actual migration |
...
The availability refresh and classification do not establish complete ecosystem migration. The ledger binds each reviewed reader candidate to its `develop` commit and blob; it does not treat unreviewed documents in an accessible repository as non-reader content. A potentially reader-facing source needs a focused migration issue before content moves or redirects; code-governing documents remain with their owner. All 19 formerly gated sources now have accessible, non-truncated develop inventories. The 27 September 2026 refresh records 169 exact revision/path/blob identities. Content review has classified all 169 paths; no paths remain pending in this refresh. Migration itself remains incomplete. The recorded decisions require focused guide migration and component-reference drift follow-ups before broader ecosystem completion can be claimed. Identical Git blobs at the same paths reuse the reviewed content decision while retaining separate repository and revision identities. Reviewed candidates include Files/BPMN integration, starter host and executable-wrapper onboarding, and the harness stub flow; their decisions do not claim completed migration. The older 23-source inventories and 100 reviewed dispositions retain their historical dates and source identities.

The current refresh records 48 reader-guide candidates and 121 retained component references. Every candidate has an owning follow-up: [#1418](https://github.com/service-lasso/service-lasso/issues/1418) covers 20 starter-host, wrapper and harness paths; [#1419](https://github.com/service-lasso/service-lasso/issues/1419) covers 14 app-owned service workflow paths; [#1420](https://github.com/service-lasso/service-lasso/issues/1420) covers 14 Broker operator paths. These scoped records carry migration decisions and verification requirements; they are not completed migrations. Component contract and template drift remains explicit in each reviewed action.

The [authoring migration decisions](authoring-migration-decisions.json) map every #1418 source path to proposed Core coverage and retain its companion-repository work explicitly. Core proposal, merge, component navigation and runtime acceptance remain separate records.
