...
| FI-012 | preferred-port collision | config/start | Cross-platform direct | existing `tests/port-reservations.test.js` service negotiation avoids API reservations | INV-2, INV-4, INV-5, INV-6, INV-7 | partial |
| FI-013 | fixed conflict and bind race after reservation | config/start | Cross-platform direct | existing `tests/port-reservations.test.js` ledger conflict plus planned post-reservation bind-race fixture | INV-2, INV-4, INV-5, INV-6, INV-7 | partial |
