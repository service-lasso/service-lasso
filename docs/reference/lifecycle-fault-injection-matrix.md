...
| FI-003 | crash after reservation, materialisation, spawn, ownership persistence, and readiness | startup transaction | Ubuntu and Windows CI direct for every formal phase | `tests/startup-hard-crash-matrix.test.js`, `tests/fixtures/startup-crash-runner.mjs`, and `.github/workflows/startup-hard-crash-matrix.yml`; see `docs/reference/startup-hard-crash-matrix.md` | INV-1, INV-2, INV-3, INV-4, INV-5, INV-6, INV-7 | implemented |
