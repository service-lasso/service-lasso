...
| Release 4 — automation | CLI and Sync are partial, Core MCP is read-only, and Broker MCP plus scheduled rotation remain planned. | Identity, approval, mutation, retry, and installed-artifact evidence must be split and proven per operation. |
