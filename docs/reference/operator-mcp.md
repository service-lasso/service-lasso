...
Only update checks and update downloads are safely cancellable. `service_lasso_cancel_operation` returns `requested`, `unsupported`, or `too_late`; a request cancellation signal uses the same safe path while the MCP request remains active. Client disconnect does not repeat or orphan the guarded mutation. After Core restart, observable surviving install, configuration, start-all, and stop-all work is reconciled through the shared runtime snapshot; other non-terminal work remains honestly `detached` until an underlying adapter can prove its result.

Terminal records are retained for 24 hours by default, with an implementation maximum of seven days and a bounded store of 48 records. Cleanup occurs during later operation reads, lists, and mutations. The operation domain is independent of experimental MCP Tasks.
