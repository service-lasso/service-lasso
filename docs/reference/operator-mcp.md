...
It builds Core and runs the complete focused MCP matrix: official SDK and Inspector initialization, notifications and discovery; stdio; OAuth and transport policy; read schemas and limits; permission profiles; confirmation and idempotency; durable operation polling and cancellation; Audit correlation; redaction; and the artifact verifier itself. `Release Qualification` has a dedicated `mcp-product-source` job and its aggregate cannot succeed unless that job passes.
...
It stages the publish payload, installs its tarball into a fresh consumer directory, imports only the installed public entrypoint, and starts the packaged runtime without a source checkout. The gate then drives the official Inspector and SDK over Streamable HTTP, performs representative reads, completes one server-confirmed service start, proves an identical idempotency replay did not repeat the mutation, stops owned processes, and starts the installed runtime entrypoint again through stdio. The `MCP Product Acceptance` workflow runs this on Windows, Linux, and macOS.

Each OS uploads exactly one 90-day `mcp-product-acceptance-<platform>-<run>-<attempt>` artifact. The file is a strict metadata record: exact candidate SHA, platform, package/archive digest, Node/SDK/Inspector/protocol versions, pass classifications, and canonical result classifications. It contains no protocol capture, command, credential, request body, raw log, configuration, environment value, local path, or secret. Each lane reads the artifact back through the GitHub API, downloads it, verifies the archive digest and sole file, validates the closed content contract, and checks exact-SHA binding, size, expiry, and retention. The aggregate independently requires one verified artifact for each OS.

`npm run demo:verify-canonical -- --host=<client-visible-host>` now reports the MCP endpoint, negotiated and supported protocol revisions, SDK version, operating mode, tool/resource counts, representative read result, and guarded-action result. In guarded mode it safely calls start for the already-running canonical `echo-service` and requires the action to report `skipped`; in the default read-only mode it reports that guarded execution was not enabled.
