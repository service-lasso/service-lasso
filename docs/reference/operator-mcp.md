...
It stages the publish payload, installs its tarball plus the pinned Inspector into a fresh consumer directory, and launches a consumer-owned driver whose working directory and module resolution are confined to that consumer tree. The gate then drives every advertised revision, the official Inspector and SDK over Streamable HTTP, performs representative reads and a strict redaction denial, completes one server-confirmed service start, proves an identical idempotency replay did not repeat the mutation, stops owned processes, and starts the installed runtime entrypoint again through stdio with the same isolated working directory. Windows identity inspection uses a direct, per-PID `System.Management` WMI query for the real creation time, executable path, and command line instead of loading the slower CIM cmdlet; the same fail-closed parser and immutable fingerprint contract remains. Hosted Windows acceptance explicitly enables an internal test hook just long enough to set a capped 60-second default, then removes the enabling flag before runtime startup. Explicit per-call deadlines still win, no identity or result is injected, and ordinary product processes retain the 15-second fail-closed default. The retained record identifies which policy the lane actually exercised. The `MCP Product Acceptance` workflow and the blocking release/publish paths run this on Windows, Linux, and macOS.
