...
- `execservice` selects the runtime/provider service
- `executable` selects the concrete executable, which may be either a local binary or a provider-exported executable key
- together they define the real runtime command contract

Example provider-backed interpretation:
- `execservice: "@node"`
- `executable: "NODE"`
- `args: ["runtime/server.js"]`

This should be interpreted as: use the `@node` runtime provider, resolve its `NODE` executable, then run `runtime/server.js` with it.
