...
directory. Dashboard uses its separate
[public-capture policy](dashboard-public-capture-policy.md): it remains
review-only unless the named policy is selected, and its dynamic operational
text is then masked before the frame can be promoted.

## Current UI tour

The approved, read-only tour frames below are regenerated by the normal capture
command. The Dashboard remains omitted until a fresh policy-controlled capture
has passed independent visual review; the policy masks Dashboard values rather
than treating password masking as sufficient protection.
