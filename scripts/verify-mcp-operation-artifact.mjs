...
const artifactId = required("MCP_OPERATION_ARTIFACT_ID");
const artifactName = required("MCP_OPERATION_ARTIFACT_NAME");
const uploadDigest = required("MCP_OPERATION_ARTIFACT_DIGEST").toLowerCase();
if (!/^(?:sha256:)?[0-9a-f]{64}$/u.test(uploadDigest)) {
  throw new Error("MCP operation upload digest must be a SHA-256 digest.");
}
...
const headers = {
  ...
  "user-agent": "service-lasso-mcp-operation-artifact-verifier",
};
...
if (!artifact) throw new Error("Uploaded MCP operation artifact was not readable through the artifact API.");
if (artifact.name !== artifactName || artifact.expired === true || Number(artifact.size_in_bytes) <= 0) {
  throw new Error("MCP operation artifact identity, expiry, or size failed readback.");
}
if (String(artifact.workflow_run?.head_sha ?? "").toLowerCase() !== candidateSha) {
  throw new Error("MCP operation artifact is not bound to the candidate SHA.");
}
if (String(artifact.digest ?? "").toLowerCase() !== expectedDigest) {
  throw new Error("MCP operation artifact API digest does not match the upload digest.");
}
...
if (!Number.isFinite(retainedDays) || retainedDays < 89) {
  throw new Error("MCP operation artifact retention is shorter than the required 90-day policy.");
}
...
if (
  evidence.contractVersion !== "service-lasso.mcp-operation-evidence.v1" ||
  evidence.issue !== 863 ||
  String(evidence.candidateSha).toLowerCase() !== candidateSha ||
  evidence.testCommand !== "npm test"
) {
  throw new Error("MCP operation artifact content is not bound to the successful candidate test.");
}
const serialized = JSON.stringify(evidence);
if (/(?:bearer\s+|authorization|cookie|password|secret\s*[:=]|token\s*[:=]|private[_-]?key|[A-Za-z]:[\\/]|file:\/\/)/iu.test(serialized)) {
  throw new Error("MCP operation artifact contains forbidden sensitive or path-like material.");
}
