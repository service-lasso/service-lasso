  ...
  for (let attempt = 1; attempt <= 4; attempt += 1) {
    try {
      ...
      if (
        ...
        throw new Error(
          `${label} download size did not match release metadata.`,
        );
      }
      return info.size;
    } catch (error) {
...
async function assertMutationRootAbsent(mutationRoot) {
  const exists = await lstat(mutationRoot)
    .then(() => true)
    .catch((error) => {
  ...
  if (exists)
    fail(
      "premature_mutation",
      "Qualification mutation root exists before every payload is verified.",
    );
}
...
async function expectGuard(code, operation, mutationRoot) {
  ...
  fail(
    ...
    `Negative guard ${code} unexpectedly accepted invalid evidence.`,
  );
}

async function runNegativeGuards({
  downloadRoot,
  mutationRoot,
  coreAsset,
  expectedCoreSha256,
  ...
  const validLines = [...checksumEntries]
...
function assertChecksumRelease(entries, release) {
  const expected = new Map([
    ...Object.values(release.platforms).map(({ asset, sha256 }) => [
      asset,
      sha256,
    ]),
  ...
  for (const [name, digest] of expected) {
    if (entries.get(name) !== digest) {
      fail(
        "checksum_release_mismatch",
        `${release.repo} checksum entry ${name} is not canonical.`,
      );
    }
  }
}
...
function pinReleasedManifest(manifest, release) {
  if (
    manifest?.id !==
    (release === ADMIN_RELEASE ? "@serviceadmin" : "@secretsbroker")
  ) {
    fail(
      "released_manifest_identity_mismatch",
      `${release.repo} service manifest identity is invalid.`,
    );
  }
  const source = manifest?.artifact?.source;
  if (source?.type !== "github-release" || source?.repo !== release.repo) {
    fail(
      "released_manifest_source_mismatch",
      `${release.repo} service manifest source is invalid.`,
    );
  }
  for (const [platform, expected] of Object.entries(release.platforms)) {
    ...
    if (
      ...
      fail(
        "released_manifest_platform_mismatch",
        `${release.repo} ${platform} manifest policy is invalid.`,
      );
    }
  }
  return {
...
async function findSingleFile(root, fileName) {
    ...
    for (const entry of await readdir(directory, { withFileTypes: true })) {
      ...
      if (entry.isSymbolicLink())
        fail(
          ...
          "Published archive contains a symbolic link.",
        );
  ...
  if (matches.length !== 1)
    fail(
      "published_binary_inventory_mismatch",
...
async function invokeCoreInstall(
  coreRoot,
  serviceId,
  servicesRoot,
  workspaceRoot,
...
async function assertCoreAcquisition(payload, release, platform) {
  ...
  if (
    ...
    fail(
      "core_acquisition_contract_invalid",
      `Published Core acquisition did not bind ${release.repo}.`,
    );
  }
  await requireRegularFile(
    artifact.archivePath,
    `${release.repo} retained archive`,
  );
  const extracted = await lstat(artifact.extractedPath).catch(() => null);
  if (!extracted?.isDirectory() || extracted.isSymbolicLink()) {
    fail(
      "core_acquisition_contract_invalid",
      `${release.repo} extraction root is invalid.`,
    );
  }
  return artifact;
}
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (body?.api?.version !== expectedVersion) {
          fail(
            "published_runtime_version_mismatch",
            "Published runtime health version is not exact.",
          );
        }
        return;
      }
...
async function stopChild(child) {
  ...
  if (!exited) {
    child.kill("SIGKILL");
    fail(
      "published_runtime_cleanup_failed",
      "Published runtime did not stop within the bounded cleanup window.",
    );
  }
}

async function runReleaseRuntimeSmoke(
  coreRoot,
  servicesRoot,
  workspaceRoot,
  version,
  ...
  try {
    ...
    if (child.exitCode !== null)
      throw new Error("Published runtime exited before readiness.");
    throw error;
  } finally {
  ...
  if (/token|secret|credential/iu.test(stdout + stderr)) {
    fail(
      "published_runtime_log_leak",
      "Published runtime smoke emitted prohibited material markers.",
    );
  }
}
...
async function runNpmConsumerSmoke({ npmTarball, consumerRoot, version }) {
  ...
  await runNpm(
    ["install", npmTarball, "--ignore-scripts", "--no-audit", "--no-fund"],
    { cwd: consumerRoot },
  );
  ...
  if (
    ...
    fail(
      "installed_npm_identity_mismatch",
      "Installed npm package identity is invalid.",
    );
  }
  const cli = path.join(packageRoot, "cli.js");
  const cliVersion = await runCommand(process.execPath, [cli, "--version"], {
    cwd: consumerRoot,
  });
  const cliHelp = await runCommand(process.execPath, [cli, "help"], {
    cwd: consumerRoot,
  });
  if (
    cliVersion.stdout.trim() !== version ||
    !cliHelp.stdout.includes("service-lasso")
  ) {
    fail(
      "installed_npm_cli_mismatch",
      "Installed npm CLI identity is invalid.",
    );
  }
...
async function appendGithubEnv(values) {
  ...
  for (const [name, value] of Object.entries(values)) {
    ...
    if (!/^[A-Z0-9_]+$/u.test(name) || /[\r\n]/u.test(normalized)) {
      fail(
        "unsafe_environment_output",
        "Qualification environment output is invalid.",
      );
    }
...
const platform = requiredEnv(
  "QUALIFICATION_PLATFORM",
  /^(?:win32|linux|darwin)$/u,
);
if (!PLATFORM_VALUES.has(platform))
  fail("invalid_platform", "Qualification platform is unsupported.");
const token = requiredEnv("GITHUB_TOKEN");
const workspace = path.resolve(requiredEnv("GITHUB_WORKSPACE"));
const runnerTemp = path.resolve(requiredEnv("RUNNER_TEMP"));
const coreReleaseId = String(
  requirePositiveInteger(
    requiredEnv("CORE_RELEASE_ID", /^[1-9][0-9]*$/u),
    "CORE_RELEASE_ID",
  ),
);
const coreTag = requiredEnv(
  "CORE_RELEASE_TAG",
  /^20[0-9]{2}\.[1-9][0-9]*\.[1-9][0-9]*-[0-9a-f]{7}$/u,
);
const coreRevision = requireSha(requiredEnv("CORE_REVISION"), "CORE_REVISION");
const coreNpmVersion = requiredEnv(
  "CORE_NPM_VERSION",
  /^20[0-9]{2}\.[1-9][0-9]*\.[1-9][0-9]*-[0-9a-f]{7}$/u,
);
const coreAsset = requiredEnv(
  "CORE_RELEASE_ASSET",
  /^service-lasso-20[0-9]{2}\.[1-9][0-9]*\.[1-9][0-9]*-[0-9a-f]{7}-(?:win32\.zip|linux\.tar\.gz|darwin\.tar\.gz)$/u,
);
const coreSha256 = requireSha256(
  requiredEnv("CORE_RELEASE_SHA256"),
  "CORE_RELEASE_SHA256",
);
const coreNpmIntegrity = requiredEnv(
  "CORE_NPM_INTEGRITY",
  /^sha512-[A-Za-z0-9+/]+={0,2}$/u,
);
const safeStatePath = path.resolve(
  requiredEnv("QUALIFICATION_SAFE_STATE_PATH"),
);
const privateStatePath = path.resolve(
  requiredEnv("QUALIFICATION_PRIVATE_STATE_PATH"),
);
...
if (coreNpmVersion !== coreTag || !coreTag.endsWith(coreRevision.slice(0, 7))) {
  fail(
    "core_publication_identity_mismatch",
    "Core release, npm version, and target revision are not one identity.",
  );
}
const expectedCoreAsset = `service-lasso-${coreTag}-${platform === "win32" ? "win32.zip" : `${platform}.tar.gz`}`;
if (coreAsset !== expectedCoreAsset)
  fail("core_asset_name_mismatch", "Core release asset name is not exact.");

const suffix = `${runId}-${runAttempt}-${platform}`;
const downloadRoot = path.join(
  runnerTemp,
  `service-lasso-published-downloads-${suffix}`,
);
const mutationRoot = path.join(
  runnerTemp,
  `service-lasso-published-mutation-${suffix}`,
);
const safeState = {
...
await writeFile(
  privateStatePath,
  `${JSON.stringify({ downloadRoot, mutationRoot }, null, 2)}\n`,
);
...
try {
  ...
  const [coreRelease, adminRelease, brokerRelease, npmMetadata, npmDistTags] =
    await Promise.all([
  ...
  const adminAssets = validateRelease(
    adminRelease,
    ADMIN_RELEASE,
    releaseServiceAssets(ADMIN_RELEASE),
  );
  const brokerAssets = validateRelease(
    brokerRelease,
    BROKER_RELEASE,
    releaseServiceAssets(BROKER_RELEASE),
  );
  requireAssetDigest(
    coreAssets.get(coreAsset),
    coreSha256,
    "Core release asset",
  );
  requireAssetDigest(
  ...
  requireAssetDigest(
    adminAssets.get("service.json"),
    ADMIN_RELEASE.manifestSha256,
    "Admin service manifest",
  );
  requireAssetDigest(
    adminAssets.get("SHA256SUMS.txt"),
    ADMIN_RELEASE.checksumSha256,
    "Admin checksum manifest",
  );
  requireAssetDigest(
    brokerAssets.get("service.json"),
    BROKER_RELEASE.manifestSha256,
    "Broker service manifest",
  );
  requireAssetDigest(
    brokerAssets.get("SHA256SUMS.txt"),
    BROKER_RELEASE.checksumSha256,
    "Broker checksum manifest",
  );
  const npmTarballUrl = validateNpmMetadata(
    npmMetadata,
    npmDistTags,
    coreNpmVersion,
    coreNpmIntegrity,
  ...
  const files = {
    ...
    adminArchive: path.join(
      downloadRoot,
      ADMIN_RELEASE.platforms[platform].asset,
    ),
    ...
    brokerArchive: path.join(
      downloadRoot,
      BROKER_RELEASE.platforms[platform].asset,
    ),
    brokerSbom: path.join(
      downloadRoot,
      BROKER_RELEASE.platforms[platform].sbom,
    ),
  ...
  await Promise.all([
    ...
    downloadReleaseAsset(
      brokerAssets.get("service.json"),
      files.brokerManifest,
      token,
      "Broker service manifest",
    ),
    downloadReleaseAsset(
      brokerAssets.get("SHA256SUMS.txt"),
      files.brokerChecksums,
      token,
      "Broker checksum manifest",
    ),
  ]);
  ...
  await verifyFileSha256(
    files.adminArchive,
    adminChecksumEntries.get(ADMIN_RELEASE.platforms[platform].asset),
    "Admin checksum-bound archive",
  );
  await verifyFileSha256(
    files.adminSbom,
    adminChecksumEntries.get(ADMIN_RELEASE.platforms[platform].sbom),
    "Admin checksum-bound SBOM",
  );
  await verifyFileSha256(
    files.brokerArchive,
    brokerChecksumEntries.get(BROKER_RELEASE.platforms[platform].asset),
    "Broker checksum-bound archive",
  );
  await verifyFileSha256(
    files.brokerSbom,
    brokerChecksumEntries.get(BROKER_RELEASE.platforms[platform].sbom),
    "Broker checksum-bound SBOM",
  );
  const adminManifest = pinReleasedManifest(
    await readJsonFile(files.adminManifest, "Admin service manifest"),
    ADMIN_RELEASE,
  );
  const brokerManifest = pinReleasedManifest(
    await readJsonFile(files.brokerManifest, "Broker service manifest"),
    BROKER_RELEASE,
  );
  ...
  const releaseManifest = await readJsonFile(
    path.join(coreRoot, "release-artifact.json"),
    "Core release manifest",
  );
  const corePackageJson = await readJsonFile(
    path.join(coreRoot, "package.json"),
    "Core release package.json",
  );
  if (
    ...
    fail(
      "extracted_core_identity_mismatch",
      "Extracted Core release identity is invalid.",
    );
  }
  const runtimeEntrypoint = path.join(
    coreRoot,
    releaseManifest.entrypoints.runtime,
  );
  ...
  if (
    ...
    fail(
      "published_core_replaced_by_harness",
      "Qualification harness changed a published Core entrypoint.",
    );
  }

  const releaseCliVersion = await runCommand(
    process.execPath,
    [cliEntrypoint, "--version"],
    { cwd: coreRoot },
  );
  const releaseCliHelp = await runCommand(
    process.execPath,
    [cliEntrypoint, "help"],
    { cwd: coreRoot },
  );
  if (
    releaseCliVersion.stdout.trim() !== coreTag ||
    !releaseCliHelp.stdout.includes("service-lasso")
  ) {
    fail(
      "published_core_cli_mismatch",
      "Published Core CLI identity is invalid.",
    );
  }
  await mkdir(smokeServicesRoot, { recursive: true });
  await runReleaseRuntimeSmoke(
    coreRoot,
    smokeServicesRoot,
    smokeWorkspaceRoot,
    coreTag,
  ...
  await writeFile(
    path.join(servicesRoot, "@serviceadmin", "service.json"),
    `${JSON.stringify(adminManifest, null, 2)}\n`,
  );
  await writeFile(
    path.join(servicesRoot, "@secretsbroker", "service.json"),
    `${JSON.stringify(brokerManifest, null, 2)}\n`,
  );
  const adminPayload = await invokeCoreInstall(
    coreRoot,
    "@serviceadmin",
    servicesRoot,
    workspaceRoot,
  ...
  const adminArtifact = await assertCoreAcquisition(
    adminPayload,
    ADMIN_RELEASE,
    platform,
  );
  const brokerPayload = await invokeCoreInstall(
    coreRoot,
    "@secretsbroker",
    servicesRoot,
    workspaceRoot,
  ...
  const brokerArtifact = await assertCoreAcquisition(
    brokerPayload,
    BROKER_RELEASE,
    platform,
  );
  const brokerBinary = await findSingleFile(
    brokerArtifact.extractedPath,
    BROKER_RELEASE.platforms[platform].binary,
  );
