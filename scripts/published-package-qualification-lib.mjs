...
export const QUALIFICATION_SCHEMA = "service-lasso.published-package-qualification.v1";
...
export const ADMIN_RELEASE = Object.freeze({
  ...
  manifestSha256: "4c120a534b8431741af21798a4b1c4ad1956efe58d944ea8b678619384721e62",
  checksumSha256: "e7bc817914051d0d9005f599c44c6c2bbf5eede557d4c3f95a0ff56feba63377",
  platforms: Object.freeze({
    win32: Object.freeze({
      asset: "@serviceadmin-win32.zip",
      sha256: "839729e1818f9ff4c0ed33e44c580c2f19582f204b4e003ef0fde9456280e31d",
    }),
    linux: Object.freeze({
      asset: "@serviceadmin-linux.tar.gz",
      sha256: "9cf43a9ee2a7881834bb3d1141cba148d02ccb8d6e69e0cb1e1ade4941e1bd71",
    }),
    darwin: Object.freeze({
      asset: "@serviceadmin-darwin.tar.gz",
      sha256: "7bdb7ca27488265e5ff00bdae3e23b53679f5423cece8495de6943ee66a781c1",
    }),
  }),
});
...
export const BROKER_RELEASE = Object.freeze({
  ...
  revision: "41f7206f90a624582fa53397304b8d6c3536b24b",
  manifestSha256: "22dc87112ac065cde067585d97968e3be503f463d63224c5d5079c4fe783c49d",
  checksumSha256: "f2dddc5c988cb3711e6135e5207fb780b9f3a6a2cbf98915c82097c0c8e9d4ce",
  platforms: Object.freeze({
    win32: Object.freeze({
      asset: "secretsbroker-win32.zip",
      binary: "secretsbroker.exe",
      sha256: "1bb86e6b03026e535c24b1dd00368022334f67d66d6f3af118251d29b5db7b8b",
    }),
    linux: Object.freeze({
      asset: "secretsbroker-linux.tar.gz",
      binary: "secretsbroker",
      sha256: "952856d8b4d82772de2e3aa2b66708a2f154565a13941f4c723216f3a2765807",
    }),
    darwin: Object.freeze({
      asset: "secretsbroker-darwin.tar.gz",
      binary: "secretsbroker",
      sha256: "735fb84fb6139ff36f25907f1f56d00cd731375b338cea1657c4944f899cf3c6",
    }),
  }),
});
...
export function requirePositiveInteger(value, label) {
  const normalized = requirePattern(value, /^[1-9][0-9]*$/u, label, "invalid_id");
...
export function parseGithubDigest(value, label) {
  ...
  if (!match) {
    fail("missing_release_digest", `${label} did not expose an exact SHA-256 digest.`);
  }
  return match[1];
}
...
function assertAssetUrls(asset, repo, tag) {
  ...
  if (asset.url !== apiUrl) {
    fail("redirected_asset_metadata", `Release asset ${asset.name} API identity is not canonical.`);
  }
  ...
  try {
    ...
    fail("redirected_asset_metadata", `Release asset ${asset.name} download identity is invalid.`);
  }
  ...
  if (
    ...
    fail("redirected_asset_metadata", `Release asset ${asset.name} download identity is not canonical.`);
  }
}

export function validateRelease(release, expected, expectedAssetNames) {
  if (String(release?.id ?? "") !== String(expected.id)) {
    fail("release_id_mismatch", `Release ${expected.repo} did not match the expected release ID.`);
  }
  if (release.tag_name !== expected.tag || release.name !== expected.tag) {
    fail("release_tag_mismatch", `Release ${expected.repo} did not match the expected tag.`);
  }
  if (release.target_commitish !== expected.revision) {
    fail("release_revision_mismatch", `Release ${expected.repo} did not target the expected revision.`);
  }
  if (release.draft === true || release.prerelease === true) {
    fail("release_not_final", `Release ${expected.repo} is not a final published release.`);
  }
  ...
  if (JSON.stringify(observedNames) !== JSON.stringify(expectedNames)) {
    fail("release_asset_inventory_mismatch", `Release ${expected.repo} asset inventory is not exact.`);
  }
  if (new Set(names).size !== names.length) {
    fail("duplicate_release_asset", `Release ${expected.repo} contains duplicate asset names.`);
  }

  const byName = new Map();
  for (const asset of assets) {
    if (!Number.isSafeInteger(asset.id) || asset.id <= 0 || !Number.isSafeInteger(asset.size) || asset.size <= 0) {
      fail("invalid_release_asset", `Release asset ${asset?.name ?? "unknown"} is empty or invalid.`);
    }
    if (asset.state !== "uploaded") {
      fail("invalid_release_asset", `Release asset ${asset.name} is not terminally uploaded.`);
    }
...
export function requireAssetDigest(asset, expectedSha256, label) {
  ...
  if (observed !== expected) {
    fail("release_digest_mismatch", `${label} API digest did not match the handed-off digest.`);
  }
  return observed;
}
...
export function parseChecksumManifest(content, expectedNames) {
  ...
  for (const line of lines) {
    ...
    if (!match) {
      fail("malformed_checksum_manifest", "Checksum manifest contains a malformed entry.");
    }
    const digest = match[1].toLowerCase();
    const name = match[2];
    if (name !== path.basename(name) || name.includes("\\") || name.includes("/")) {
      fail("redirected_checksum_entry", "Checksum manifest contains a redirected filename.");
    }
    if (!expected.has(name)) {
      fail("unexpected_checksum_entry", `Checksum manifest contains unexpected asset ${name}.`);
    }
    if (entries.has(name)) {
      fail("duplicate_checksum_entry", `Checksum manifest contains duplicate asset ${name}.`);
    }
  ...
  for (const name of expected) {
    if (!entries.has(name)) {
      fail("missing_checksum_entry", `Checksum manifest is missing asset ${name}.`);
    }
  }
  if (entries.size !== expected.size) {
    fail("checksum_inventory_mismatch", "Checksum manifest inventory is not exact.");
  }
  return entries;
}

export async function digestFile(filePath, algorithm = "sha256", encoding = "hex") {
...
export async function verifyFileSha256(filePath, expectedSha256, label) {
  ...
  if (actual !== expected) {
    fail("download_digest_mismatch", `${label} download did not match its expected SHA-256.`);
  }
  return { sha256: actual, size: info.size };
}

export function parseSha512Integrity(value) {
  const match = /^sha512-([A-Za-z0-9+/]+={0,2})$/u.exec(String(value ?? "").trim());
  if (!match) {
    fail("invalid_npm_integrity", "npm package integrity is not exact SHA-512 SRI.");
  }
  return match[1];
}

export function validateNpmMetadata(versionMetadata, distTags, expectedVersion, expectedIntegrity) {
  if (versionMetadata?.name !== PACKAGE_NAME || versionMetadata?.version !== expectedVersion) {
    fail("npm_identity_mismatch", "npm package identity did not match the expected package and version.");
  }
  if (distTags?.latest !== expectedVersion) {
    fail("npm_latest_mismatch", "npm latest did not match the exact published Core version.");
  }
  if (versionMetadata?.dist?.integrity !== expectedIntegrity) {
    fail("npm_integrity_mismatch", "npm registry integrity did not match the handed-off integrity.");
  }
  ...
  if (tarball.protocol !== "https:" || tarball.hostname !== "registry.npmjs.org") {
    fail("npm_tarball_redirected", "npm tarball URL is not bound to the public npm registry.");
  }
  return tarball.href;
}
...
export async function verifyNpmTarballIntegrity(filePath, expectedIntegrity) {
  ...
  if (actualBase64 !== expectedBase64) {
    fail("npm_tarball_integrity_mismatch", "npm tarball bytes did not match registry integrity.");
  }
...
export function safeFailureCode(error) {
  if (error instanceof QualificationError && /^[a-z0-9_]{1,64}$/u.test(error.code)) {
    return error.code;
  }
  return "qualification_failed";
}

export function validateRetainedArtifactMetadata(artifact, expected, now = Date.now()) {
  ...
  if (
    ...
    fail("invalid_retained_artifact", "Retained artifact is missing, empty, expired, wrong-head, or outside policy.");
  }
  return artifact;
}
...
export function validateTerminalJobMetadata(job, expected) {
  if (
    ...
    job?.url !== `https://api.github.com/repos/${expected.repo}/actions/jobs/${job.id}` ||
    job?.run_url !== `https://api.github.com/repos/${expected.repo}/actions/runs/${expected.runId}` ||
    job?.html_url !== `https://github.com/${expected.repo}/actions/runs/${expected.runId}/job/${job.id}`
  ) {
    fail("invalid_terminal_job", "Terminal job is missing, non-green, or wrong-head.");
  }
  return job;
}

const FORBIDDEN_EVIDENCE_KEY = /(path|url|token|secret|credential|command|args|claim|environment|store|vault|ipc|endpoint|host|port)/iu;
...
export function assertMetadataOnlyEvidence(value, trail = "evidence") {
  ...
  if (Array.isArray(value)) {
    value.forEach((entry, index) => assertMetadataOnlyEvidence(entry, `${trail}[${index}]`));
    return;
  }
...
export function validateRetainedEvidence(evidence, expected) {
  ...
  if (evidence?.schema !== QUALIFICATION_SCHEMA || evidence?.retainedContent !== "metadata_only") {
    fail("invalid_evidence_schema", "Retained evidence schema is invalid.");
  }
  if (evidence.outcome !== "success" || evidence.platform !== expected.platform) {
    fail("qualification_not_successful", `Retained ${expected.platform} outcome is not successful.`);
  }
  if (
    ...
    fail("evidence_run_mismatch", `Retained ${expected.platform} run identity is invalid.`);
  }
  if (
    ...
    fail("evidence_core_mismatch", `Retained ${expected.platform} Core identity is invalid.`);
  }
  for (const [name, release] of [["admin", ADMIN_RELEASE], ["broker", BROKER_RELEASE]]) {
    ...
    if (
      ...
      fail("evidence_dependency_mismatch", `Retained ${expected.platform} ${name} identity is invalid.`);
    }
  }
  if (evidence.retentionDays !== RETENTION_DAYS || evidence.mutationRetry !== false) {
    fail("evidence_policy_mismatch", `Retained ${expected.platform} policy evidence is invalid.`);
  }
  ...
  if (
    ...
    requiredNegativeProof.some((name) => evidence.negativeProof[name] !== "success")
  ) {
    fail("evidence_negative_proof_incomplete", `Retained ${expected.platform} negative proof is incomplete.`);
  }
  if (evidence.mutations?.brokerRestart !== 1 || evidence.mutations?.providerMigrationApply !== 1) {
    fail("evidence_mutation_mismatch", `Retained ${expected.platform} mutation-count evidence is invalid.`);
  }
  ...
  if (expected.platform === "win32") requiredScenarios.push("localOperatorLockout");
  for (const scenario of requiredScenarios) {
    if (evidence.scenarios?.[scenario] !== "success") {
      fail("evidence_scenario_incomplete", `Retained ${expected.platform} scenario ${scenario} is incomplete.`);
    }
  }
  return evidence;
}
