...
if (!existsSync(path.join(data, 'PG_VERSION'))) {
  ...
  try {
    const initialized = spawnSync(executable('initdb'), ['-D', data, '-U', process.env.POSTGRES_USER, '--encoding', 'UTF8', '--pwfile', passwordFile, '--auth-host=scram-sha-256'], { stdio: 'inherit' });
...
const child = spawn(executable('postgres'), ['-D', data, '-h', '127.0.0.1', '-p', process.env.POSTGRES_PORT, '-c', `max_connections=${process.env.POSTGRES_MAX_CONNECTIONS ?? '100'}`], { stdio: 'inherit' });
