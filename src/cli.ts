...
interface ParsedCliOptions {
  command: "serve" | "install" | "start" | "setup" | "updates" | "recovery" | "health" | "help" | "version";
...
function parseCliArgs(argv: string[]): ParsedCliOptions {
  ...
  while (remaining.length > 0) {
    ...
    switch (token) {
      ...
      case "--json": {
        if (command !== "install" && command !== "start" && command !== "setup" && command !== "updates" && command !== "recovery" && command !== "health") {
          throw new Error("--json is only supported for the install, start, setup, updates, recovery, and health commands.");
        }
        parsed.json = true;
        break;
      }
