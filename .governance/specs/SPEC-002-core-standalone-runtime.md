...
- 2026-04-27: `#171` hardens `service-lasso/lasso-traefik` as the current release-service reference pattern. Release `2026.4.27-354433e` published Windows/Linux/macOS Traefik archives, released `service.json`, and `SHA256SUMS.txt`; `#185` then aligned the Traefik manifest with donor-style HTTP readiness and pinned core `@traefik` to release `2026.4.27-38bd54d`, with coverage from `npm run verify:traefik-release`.
