...
- 2026-04-28: `#201` makes `service-admin` and `localcert` explicit core services in checked-in manifests and docs. `service-admin` remains a release-backed managed core UI service, while `localcert` remains a local/no-download provider-role core utility until fuller certificate materialization is implemented.
