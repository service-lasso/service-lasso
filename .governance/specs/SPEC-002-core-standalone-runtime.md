...
- `AC-4Z`: A clean clone of `service-lasso` has one documented baseline start path that can acquire, configure, and start the expected baseline services from their canonical service repos where downloads are required: `@localcert`, `@nginx`, `@traefik`, `@node`, `echo-service`, and `@serviceadmin`. Any baseline service that is intentionally local/no-download must be explicitly documented as such, and any missing capability must be tracked as a release-readiness blocker before core completion is claimed.
...
- `AC-4AJ`: The core repo provides a buildable in-repo documentation site and an agent-ready service-authoring guide so a contributor can create a new `service-lasso/lasso-*` service repo, release artifacts, and pin the resulting `service.json` in consumers without reconstructing the process from chat history.
...
- direct proof that the in-repo documentation site builds, links the existing canonical docs, and includes an agent-ready new-service guide covering service identity, `service.json`, artifact naming, release workflow, validation, and consumer pinning
...
- 2026-04-27: `#171` hardens `service-lasso/lasso-traefik` as the current release-service reference pattern. Release `2026.4.27-354433e` published Windows/Linux/macOS Traefik archives, released `service.json`, and `SHA256SUMS.txt`; `#185` aligned the Traefik manifest with donor-style HTTP readiness, `#187` restored Traefik `env` / `globalenv` outputs, `#189` restored the full donor-style Traefik service-port map, and `#191` preserved the explicit donor `portmapping` contract while pinning core `@traefik` to release `2026.4.27-41ef504`.
- 2026-04-27: `#193` adds first-class manifest `commandline` support for donor-style startup arguments. The runtime validates platform commandline maps, resolves service variables into the selected platform/default commandline at start/restart time, parses it into process args, and pins `@traefik` to release `2026.4.27-b879d28`, whose manifest carries the donor-style Traefik provider/dashboard/entrypoint/transport flags.
- 2026-04-27: `#195` restores Traefik's missing donor-aligned dependencies by releasing `service-lasso/lasso-traefik@2026.4.27-bbc7f15` with local certificate and NGINX dependency edges, adding core dependency manifests, and extending baseline bootstrap so those dependencies install/configure before Traefik. The NGINX marker is superseded by the managed release-backed service in `#198`.
...
- 2026-04-27: `#204` creates `service-lasso/lasso-localcert` as the release-backed local certificate utility provider. Release `2026.4.27-591ed28` publishes Windows/Linux/macOS archives, released `service.json`, and `SHA256SUMS.txt`; core pins that release through `@localcert` and exports donor-aligned `CERT_FILE`, `CERT_KEY`, `CERT_PFX`, and `CAROOT_CERT` globals from the installed artifact root.
