...
- Canonical demo network identity is operator-owned configuration rather than a checked-in machine address. Deploy requires an explicit bind host and either a client-visible URL host or complete runtime/Admin URLs; verifier and watchdog require a shared host unless their complete URLs are supplied. Internal loopback-only service health and ownership checks remain explicit local contracts (`SPEC-002` `AC-4N.1`). Loopback canonical recycle completes first-run vault bootstrap and confirmed `startAll` when setup mode would block autostart, without logging secret values or converting a failed recycle into a pass (`SPEC-002` `AC-4N.2`). Canonical demo verification unwraps v2 runtime-instance envelopes and treats MCP connect failures as not-ready so recycle can wait until the selected generation is listening (`SPEC-002` `AC-4BI`).
...
- In-process CLI, leftover durable CLI commands, scheduler, and recovery-monitor durable mutations resolve an explicit actor, reuse the HTTP permission helper and audit store, deny ungranted or unconfirmed sensitive work without mutation, and never persist secret values (`SPEC-002` `AC-4CC`).
...
- Service isolation is a ladder on Core-supervised processes: default direct mode, optional workspace/limits/user/hardening/compose-scripts rungs, no Core Docker provider in this requirement, honest degrade/`require` reporting, and Admin status without a Docker UI (`SPEC-002` `AC-4CE`).
