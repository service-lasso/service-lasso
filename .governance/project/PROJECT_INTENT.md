...
- Service lifecycle mutations must derive authority from the trusted runtime request context, enforce the same permission and confirmation decisions projected to Service Admin, and keep cross-repository acceptance ordered through the Admin lifecycle-action UI sublane and its three-OS qualification, an exact Admin release, the Core pin, and packaged proof (`SPEC-002` `AC-4BY`).
