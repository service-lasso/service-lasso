...
- Service lifecycle mutations must derive authority from the trusted runtime request context, enforce the same permission and confirmation decisions projected to Service Admin, and keep cross-repository acceptance ordered through the Admin lifecycle-action UI sublane and its three-OS qualification, an exact Admin release, the Core pin, and packaged proof. That packaged proof must retain authenticated protected Broker transport across target readiness reads and a single durable provider-migration apply without mutation retry or secret-bearing evidence (`SPEC-002` `AC-4BY`, `AC-4BY.1`).
