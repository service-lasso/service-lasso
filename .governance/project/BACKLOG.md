...
| `TASK-034` | `in_progress` | `ISS-034` | Run `lasso-@serviceadmin` integration validation against the current runtime/API | `SPEC-002`, `AC-4O` | The admin UI can consume the current runtime/API against the bounded demo/runtime shape without special-case hacks, starting with service meta persistence, live log-read compatibility, and dashboard/service-detail contract proof; any discovered gaps are recorded as governed follow-up work |
