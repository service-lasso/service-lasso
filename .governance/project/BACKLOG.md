...
| `ISS-034` | `in_progress` | Validate `lasso-@serviceadmin` against the current runtime/API | `SPEC-002`, `AC-4O` | Use the current bounded runtime plus released Echo Service artifacts as the first real consumer integration proof before package/template rollout. Start with service meta persistence, live log-read compatibility routes, and bounded dashboard adapter routes, then switch the admin consumer off the large stub model and record any remaining gaps. |
...
| `TASK-034` | `in_progress` | `ISS-034` | Run `lasso-@serviceadmin` integration validation against the current runtime/API | `SPEC-002`, `AC-4O` | The admin UI can consume the current runtime/API against the bounded demo/runtime shape without special-case hacks, starting with service meta persistence, live log-read compatibility, and bounded dashboard summary/services/detail contract proof; any discovered gaps are recorded as governed follow-up work |
